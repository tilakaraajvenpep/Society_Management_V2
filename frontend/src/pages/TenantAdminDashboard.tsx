...
const TenantAdminDashboard = () => {
  ...
  const getDiscountForPayment = (months: number, selectedCategory: string = 'Maintenance', payDate: string = getTodayDateString(), coverageStart: string = '', coverageEnd: string = '', baseAmt: number = 0) => {
    if (selectedCategory !== 'Maintenance') return 0;
    if (!summary.discountDate || !summary.discountAmount || !payDate) return 0;

    let totalMonths = 0;
    if (months > 0) {
      totalMonths = getMonthsCount(months, coverageStart, coverageEnd);
    } else if (coverageStart && coverageEnd) {
      totalMonths = getMonthsCount(0, coverageStart, coverageEnd);
    } else if (baseAmt > 0) {
      totalMonths = Math.max(1, Math.round(baseAmt / (summary.maintenanceAmount || 1)));
    } else {
      return 0;
    }
  ...
  const getLateFeeForPayment = (months: number, selectedCategory: string = 'Maintenance', payDate: string = getTodayDateString(), coverageStart: string = '', coverageEnd: string = '', baseAmount: number = 0) => {
    if (selectedCategory !== 'Maintenance') return 0;
    if (months === 0 && !baseAmount && !(coverageStart && coverageEnd)) return 0;
    if (!summary.lateFeeDate || !summary.lateFeeAmount || !payDate) return 0;
  ...
  const getPaymentBaseAndLateFee = (payment: any) => {
    ...
    if (payment.category === 'Maintenance' && summary.lateFeeDate && summary.lateFeeAmount && payment.paymentDate) {
      const cutOffDay = getDayFromDateString(summary.lateFeeDate);
  ...
  const handleOpenEditPayment = (payment: any) => {
    const { baseAmount, lateFee } = getPaymentBaseAndLateFee(payment);
    setEditingPayment({
      id: payment.id,
                    ...
                    <div className="grid-2">
                      <div>
                        ...
                        <select required value={newPayment.memberId} onChange={(e) => {
                          ...
                          const discountAmt = getDiscountForPayment(newPayment.paidMonths, newPayment.category, newPayment.paymentDate, newPayment.coverageStartDate || '', newPayment.coverageEndDate || '', newPayment.amount);
                          setNewPayment({ 
                            ...newPayment, 
                      ...
                      <div>
                        ...
                        <select required value={newPayment.category} onChange={(e) => {
                          ...
                          const discountAmt = getDiscountForPayment(newPayment.paidMonths, cat, newPayment.paymentDate, newPayment.coverageStartDate || '', newPayment.coverageEndDate || '', newPayment.amount);
                          setNewPayment({ 
                            ...newPayment, 
                            category: cat,
                      ...
                      {newPayment.category === 'Maintenance' ? (
                        <div>
                          ...
                          <select required value={newPayment.paidMonths} onChange={(e) => {
                            ...
                            const discountAmt = getDiscountForPayment(months, newPayment.category, newPayment.paymentDate, newPayment.coverageStartDate || '', updatedEndDate || '', baseAmt);
                            setNewPayment({ 
                              ...newPayment, 
                              paidMonths: months, 
                              periodLabel: label, 
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          onChange={(e) => {
                            ...
                            const discountAmt = getDiscountForPayment(newPayment.paidMonths, newPayment.category, newDate, newPayment.coverageStartDate || '', newPayment.coverageEndDate || '', newPayment.amount);
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '1rem', marginBottom: '1rem' }}>
                        ...
                        <input 
                          ...
                          onChange={(e) => {
                            ...
                            if (/^\d*\.?\d*$/.test(val)) {
                              ...
                              const discountAmt = getDiscountForPayment(newPayment.paidMonths, newPayment.category, newPayment.paymentDate, newPayment.coverageStartDate || '', newPayment.coverageEndDate || '', amt);
                      ...
                      <div>
                        ...
                        <input 
                          type="text" 
                          disabled 
                          value={newPayment.amount + newPayment.lateFee - newPayment.discount} 
                          style={{
                            backgroundColor: 'var(--bg-secondary)',
                            color: 'var(--text-primary)',
                            fontWeight: 'bold',
                      ...
                      <div className="grid-2" style={{ backgroundColor: 'var(--bg-secondary)', padding: '1rem', borderRadius: '0.5rem', border: '1px dashed var(--border-color)' }}>
                        ...
                        <div>
                          ...
                          <input type="date" value={newPayment.coverageStartDate || ''} onChange={(e) => {
                            ...
                            const discountAmt = getDiscountForPayment(newPayment.paidMonths, newPayment.category, newPayment.paymentDate, startDate, updatedEndDate, newPayment.amount);
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          onChange={(e) => {
                            ...
                            const fee = getLateFeeForPayment(editingPayment.paidMonths, editingPayment.category, newDate, editingPayment.coverageStartDate || '', editingPayment.coverageEndDate || '', baseAmt);
                            setEditingPayment({ 
                              ...editingPayment, 
                              paymentDate: newDate,
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '1rem' }}>
                      <div>
                        ...
                        <input 
                          type="text" 
                          required 
                          value={editingPayment.baseAmount === 0 ? '' : editingPayment.baseAmount} 
                          onChange={(e) => {
                            const val = e.target.value;
                            if (/^\d*\.?\d*$/.test(val)) {
                              const baseAmt = val === '' ? 0 : parseFloat(val);
                              const fee = getLateFeeForPayment(editingPayment.paidMonths, editingPayment.category, editingPayment.paymentDate, editingPayment.coverageStartDate || '', editingPayment.coverageEndDate || '', baseAmt);
                              setEditingPayment({ 
                                ...editingPayment, 
                                baseAmount: baseAmt,
                                amount: baseAmt + fee
                      ...
                      <div>
                        ...
                        <input 
                          type="text" 
                          disabled 
                          value={getLateFeeForPayment(editingPayment.paidMonths, editingPayment.category, editingPayment.paymentDate, editingPayment.coverageStartDate || '', editingPayment.coverageEndDate || '', editingPayment.baseAmount)} 
                          style={{
                            backgroundColor: 'var(--bg-secondary)',
                    ...
                    <div className="grid-2" style={{ backgroundColor: 'var(--bg-secondary)', padding: '1rem', borderRadius: '0.5rem', border: '1px dashed var(--border-color)' }}>
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          onChange={(e) => {
                            const startDate = e.target.value;
                            const baseAmt = getBaseAmount(editingPayment.paidMonths, editingPayment.category, editingPayment.paymentDate, startDate, editingPayment.coverageEndDate || '');
                            const fee = getLateFeeForPayment(editingPayment.paidMonths, editingPayment.category, editingPayment.paymentDate, startDate, editingPayment.coverageEndDate || '', baseAmt);
                            setEditingPayment({ 
                              ...editingPayment, 
                              coverageStartDate: startDate, 
                              baseAmount: editingPayment.category === 'Maintenance' ? baseAmt : editingPayment.amount,
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          onChange={(e) => {
                            const endDate = e.target.value;
                            const baseAmt = getBaseAmount(editingPayment.paidMonths, editingPayment.category, editingPayment.paymentDate, editingPayment.coverageStartDate || '', endDate);
                            const fee = getLateFeeForPayment(editingPayment.paidMonths, editingPayment.category, editingPayment.paymentDate, editingPayment.coverageStartDate || '', endDate, baseAmt);
                            setEditingPayment({ 
                              ...editingPayment, 
                              coverageEndDate: endDate, 
                              baseAmount: editingPayment.category === 'Maintenance' ? baseAmt : editingPayment.amount,
